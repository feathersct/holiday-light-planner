...
2. **Explore ▾** — dropdown containing:
   - Map
   - Organizers
   - *(Admin — visible to admin users only, moves here from its current flat nav position)*
   - *(Filters — future, not implemented now)*
3. Right side — **avatar** (logged in) or **Sign In** button (logged out)

**Avatar** (logged in): currently decorative; becomes a clickable dropdown trigger. Dropdown contains:
- My Account
- Sign Out

The navbar no longer contains standalone "Add Display", "My Displays", "Sign Out", or "⚙ Admin" buttons.
...
- Avatar tap opens a **bottom sheet** (new component to build) containing:
  - My Account
  - Sign Out
...
| ⚙ Admin (flat nav link) | Admin | Explore dropdown (admin users only) |
